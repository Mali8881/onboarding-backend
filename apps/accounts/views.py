...
from apps.content.models import Course, CourseEnrollment
from apps.reports.models import EmployeeDailyReport
from apps.onboarding_core.models import OnboardingDay, OnboardingProgress
from apps.onboarding_core.services import ensure_day_two_task_for_intern
    ...
    def post(self, request):
        ...
        if user:
            ...
            if user.email:
                ...
                try:
                    send_mail(
                        subject="Сброс пароля — В Плюсе",
                        message=(
                            f"Здравствуйте, {user.first_name or user.username}!\n\n"
                            f"Вы запросили сброс пароля на платформе «В Плюсе».\n\n"
                    ...
                    pass  # не блокируем ответ при ошибке почты
...
class RoleListCreateAPIView(_OrgAdminMixin, ListCreateAPIView):
    serializer_class = RoleManageSerializer

    def get_queryset(self):
        return Role.objects.prefetch_related("permissions").annotate(users_count=Count("user")).order_by("-level", "name")

    def list(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            raise PermissionDenied("Требуется авторизация.")
        if not AccessPolicy.is_admin_like(request.user):
            raise PermissionDenied("Only admin-like roles can view roles.")
        return super().list(request, *args, **kwargs)

    def create(self, request, *args, **kwargs):
        if not AccessPolicy.has_permission(request.user, "roles_manage"):
            raise PermissionDenied("Missing permission: roles_manage.")
        return super().create(request, *args, **kwargs)


class RoleDetailAPIView(_OrgAdminMixin, RetrieveUpdateDestroyAPIView):
    serializer_class = RoleManageSerializer
    queryset = Role.objects.prefetch_related("permissions").annotate(users_count=Count("user"))
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if User.objects.filter(role=instance).exists():
            return Response(
                {"detail": "Нельзя удалить роль: она назначена пользователям."},
                status=status.HTTP_409_CONFLICT,
            )
        if instance.name in set(Role.Name.values):
            return Response(
